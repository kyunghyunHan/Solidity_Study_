...
## 계약 함수가 호출되는 방법
- 이더리움트래잭션에는 data라는 필드가 존재
- 이더를 보낼때는 비어 있어야 하지만 계약과 상호작용 할때는 무언가를 포함해야한다
...
```
- solidity에는 컴파일한 모든 스마트 계약에는 호출데이터를 구문 분석하고 호출데이터에서 추출한 함수 식별자에 따라 호출할 함수를 결정하는 분기논리가 있다
...
- calldatasize호출 데이터 크기를 가져오기 위해 함수를 사용하고 인덱스에 있는 슬롯에 특정 크기의 호출데이터를 복사하는데 사용하는 calldatacopy
- ptr:맞지 않으면 다른 슬롯을 차지
- mload:지정된 인덱스에서 32바이트를 읽는 함수
- index 0x40:은 다음 여유 메모리 슬롯의 인덱스를 포함하는 특수슬롯을 가리킨다, 따라서 기본적으로 통화 데이터를 여유 메모리 슬롯에 저장합니다
2. 다음으로 호출을 중계합니다.
- delegatecall 다른 계약을 호출하는 까다로운 방법
- 첫번떄 인수 delegatecall은 현재 호출에 남아있는 가스의 양 , 이것은 다른 계약에 사용할수 있는 가스의 양을 알려쥽니다
- implementation상태변수:코드가 실행될떄 메모리에 저장
- .slot접미사는 solidity의 기능,상태변수의 슬롯 주소를 쉽게 얻을수 있다
- sload:그 주소가 있으면 그주소의 값을 읽는 함수를 호출
- 0 인 마지막 두 인수는 각각 out 및 outsize반환 데이터를 저장할 메모리위치를 정의
